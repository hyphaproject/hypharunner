...
void CommunicateHandler::handleRequest(Poco::Net::HTTPServerRequest &request, Poco::Net::HTTPServerResponse &response) {
    try {
        std::string message = "";
        ...
        if (request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET) {
            Logger::info(request.clientAddress().toString() + " connected to CommunicateHandler_Get.");
            Poco::Net::HTMLForm form(request);
            message = form["message"];
        } else {
            Logger::info(request.clientAddress().toString() + " connected to CommunicateHandler_Post.");
