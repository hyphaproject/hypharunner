...
void Video::doWork() {
    ...
    try {
        ...
        if(findPerson)
            findPersons();
        std::thread concatDrawT = std::thread( [this] { concatImages(); drawText(); });
...
void Video::captureCamera(int camNumber) {
...
cv::Mat Video::rotateMat(cv::Mat mat) {
    if(mat.rows < 1)
        return mat;
...
cv::Mat Video::fillMat(cv::Mat mat) {
    if(mat.rows != height)
...
void Video::drawText() {
    try {
        ...
        cv::putText(frame, id + " " + time, cv::Point(30,30),
                    cv::FONT_HERSHEY_COMPLEX_SMALL, 1, cv::Scalar(20,20,250), 1, CV_AA);
        currentImage_mutex.lock();
...
void Video::findPersonInCamera(int camNumber) {
    if(findPerson_mutex[camNumber].try_lock()) {
        try {
            ...
            if(hog_mutex[camNumber].try_lock()) {
                ...
                hog[camNumber].detectMultiScale(image, found, 0, cv::Size(8,8), cv::Size(32,32), 1.05, 2);
...
std::string Video::communicate(std::string message) {
    return "SUCCESS";
}
...
HyphaPlugin *Video::getInstance(std::string id) {
...
void Video::startVideoCapture() {
    try {
        ...
        // WMV2
        if(!videoWriter->open(filename+".avi", CV_FOURCC('M','P','4','2'), fps, cv::Size(frame.cols, frame.rows))) {
            //if(!videoWriter->open(filename+".wmv", CV_FOURCC('W','M','V','2'), fps, cv::Size(frame.cols, frame.rows))){
