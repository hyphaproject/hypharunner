...
std::string RpiPir::communicate(std::string message) {
...
void RpiPir::sendMovement() {
    ...
    if (movement) {
        if (log)
            Logger::info("Movement detected: " + this->getId());
...
hypha::plugin::HyphaPlugin *RpiPir::getInstance(std::string id) {
    RpiPir *instance = new RpiPir();
    instance->setId(id);
    return instance;
}

void RpiPir::receiveMessage(std::string message) {
}
