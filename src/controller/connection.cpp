...
Connection *Connection::factory(std::string handlerId, std::string pluginId) {
  if ((PluginSettings::instance()->getHost(pluginId) == Poco::Net::DNS::hostName() || PluginSettings::instance()->getHost(pluginId) == "localhost")
      && (HandlerSettings::instance()->getHost(handlerId) == Poco::Net::DNS::hostName() || HandlerSettings::instance()->getHost(handlerId) == "localhost")) {
...
std::string Connection::communicate(std::string id, std::string message) {
  ...
  if (handler) {
    if (HandlerSettings::instance()->getHost(id) == Poco::Net::DNS::hostName() || HandlerSettings::instance()->getHost(id) == "localhost") {
  ...
  if (plugin) {
    if (PluginSettings::instance()->getHost(id) == Poco::Net::DNS::hostName() || PluginSettings::instance()->getHost(id) == "localhost") {
      return LocalConnection::communicate(id, message);
    } else {
      return NetworkConnection::communicate(id, message);
    }
