...
void BlechUhr::parse(std::string message) {
  ...
  if (object.contains("devices")) {
    foreach (QVariant device,
             object.value("devices").toArray().toVariantList()) {
    ...
    std::string device =
        "authuser" + object.value("authuser").toString().toStdString();
...
void BlechUhr::storeDevice(std::string device, std::string type) {
  ...
  std::string time = QDateTime::currentDateTime()
                         ...
                         .toStdString();
  statement
      << "insert into deviceonline(deviceid, time, type) values(?, ?, ?);",
      Poco::Data::Keywords::use(device), Poco::Data::Keywords::use(time),
...
void BlechUhr::giveFeedback(std::string username, std::string device) {
  ...
  if (username.empty()) {
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(400));
    sendMessage(
        "{\"red\":false,\"green\":false,\"yellow\":false,\"beep\":200, "
        "\"door\":false}");
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(400));
    sendMessage(
        "{\"green\":false,\"green\":false,\"yellow\":false, \"door\":false}");
...
int BlechUhr::countDeviceToday(std::string device) {
  int retValue = 0;
  Poco::Data::Statement statement = Database::instance()->getStatement();
  statement << "select count(id) from deviceonline where deviceid='" + device +
                   "' and DATE(time) = '" +
...
void BlechUhr::loadConfig(std::string UNUSED(config)) {}
...
void BlechUhr::receiveMessage(std::string message) { parse(message); }

std::string BlechUhr::communicate(std::string UNUSED(message)) { return ""; }
