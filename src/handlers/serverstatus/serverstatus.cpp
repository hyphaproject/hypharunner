...
void ServerStatus::receiveMessage(std::string message) {
}

std::string ServerStatus::communicate(std::string message) {
  return "";
}

void ServerStatus::sendEMail() {
  std::string message = "Server Status\n\n";
  for (std::string pluginId : HandlerLoader::instance()->getConnectedPlugins(id)) {
    HyphaPlugin *p = PluginLoader::instance()->getPluginInstance(pluginId);
    if (p) {
      message += "Component of Type " + p->name() + " with ID " + p->getId() + ":\n";
