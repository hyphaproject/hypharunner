    ...
    foreach (QVariant device,
      ...
      if (!device.toString().isEmpty()) {
        if (device.toString().toStdString() == mastercard) {
          ...
          std::string username = UserDatabase::instance()->getOwnerOfDevice(
              device.toString().toStdString());
          if (useFingerprint) {
...
void DoorOpener::loadConfig(std::string config) {
  QJsonDocument document =
      QJsonDocument::fromJson(QString::fromStdString(config).toUtf8());
  QJsonObject object = document.object();
  ...
  while (more) {
    if (rs[0].convert<bool>()) {
      ...
      while (more) {
        ...
        QDate start = QDate::fromString(
            QString::fromStdString(rs[1].convert<std::string>()));
        QDate end = QDate::fromString(
            QString::fromStdString(rs[2].convert<std::string>()));
        QTime starttime = QTime::fromString(
            QString::fromStdString(rs[3].convert<std::string>()));
        QTime endtime = QTime::fromString(
            QString::fromStdString(rs[4].convert<std::string>()));
        Logger::warning(type);
